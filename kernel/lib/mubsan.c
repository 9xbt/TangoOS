...
void __ubsan_handle_type_mismatch_v1(mubsan_type_mismatch_info_v1* data, uintptr_t ptr) {
    ...
    mubsan_log("mubsan @ %s:%d:%d: %s, %s type %s at alignment %d at address 0x%x\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               reason,
...
void __ubsan_handle_add_overflow(mubsan_overflow* data, uintptr_t lhs, uintptr_t rhs) {
    mubsan_log("mubsan @ %s:%d:%d: addition overflow, for type %s, expression %d + %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->type->name,
...
void __ubsan_handle_sub_overflow(mubsan_overflow* data, uintptr_t lhs, uintptr_t rhs) {
    mubsan_log("mubsan @ %s:%d:%d: subtraction overflow, for type %s, expression %d - %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->type->name,
...
void __ubsan_handle_mul_overflow(mubsan_overflow* data, uintptr_t lhs, uintptr_t rhs) {
    mubsan_log("mubsan @ %s:%d:%d: multiplication overflow, for type %s, expression %d * %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->type->name,
...
void __ubsan_handle_negate_overflow(mubsan_overflow* data, uintptr_t val) {
    mubsan_log("mubsan @ %s:%d:%d: negate overflow, for type %s, value %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->type->name,
...
void __ubsan_handle_divrem_overflow(mubsan_overflow* data, uintptr_t lhs, uintptr_t rhs) {
    mubsan_log("mubsan @ %s:%d:%d: divistion overflow, for type %s, expression %d / %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->type->name,
...
void __ubsan_handle_pointer_overflow(mubsan_pointer_overflow* data, uintptr_t base, uintptr_t result) {
    mubsan_log("mubsan @ %s:%d:%d: pointer overflow, base 0x%x, result 0x%x\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               base,
...
void __ubsan_handle_out_of_bounds(mubsan_out_of_bounds* data, uintptr_t index) {
    mubsan_log("mubsan @ %s:%d:%d: array out of bounds, for type %s, by index type %s %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
...
void __ubsan_handle_nonnull_arg(mubsan_not_null_arg* data) {
    mubsan_log("mubsan @ %s:%d:%d: not-null argument is null\n",
               data->loc.file,
               data->loc.line,
               data->loc.col);
}
void __ubsan_handle_load_invalid_value(mubsan_invalid_value* data, uintptr_t val) {
    mubsan_log("mubsan @ %s:%d:%d: load of invalid value, for type %s, value %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->type->name,
...
void __ubsan_handle_shift_out_of_bounds(mubsan_shift_out_of_bounds* data, uintptr_t lhs, uintptr_t rhs) {
    mubsan_log("mubsan @ %s:%d:%d: shift out of bounds, of type %s and %s, value %d and %d\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
               data->lhs_type->name,
...
void __ubsan_handle_builtin_unreachable(mubsan_unreachable* data) {
    mubsan_log("mubsan @ %s:%d:%d: unreachable code was reached\n",
               data->loc.file,
               data->loc.line,
               data->loc.col);
}
void __ubsan_handle_function_type_mismatch(mubsan_function_type_mismatch* data) {
    mubsan_log("mubsan @ %s:%d:%d: function type mismatch, expected %s but got %s at address %x\n",
               data->loc.file,
               data->loc.line,
               data->loc.col,
